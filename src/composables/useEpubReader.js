    ...
    setTimeout(() => {
      ...
      if (isPageMode) {
        ...
        if (position.currentPage !== undefined && position.displayMode === 'page' && !position.switchingMode) {
          ...
          // Convert scroll ratio to page (mode switch or ratio-based restore)
  ...
  function setupDisplayModeWatcher() {
    ...
    displayModeWatcher = watch(
      ...
      async (newMode, oldMode) => {
        ...
        if (savedFile) {
          // Store position ratio for restore (mode-independent)
          // This will be converted to the appropriate format in restorePosition
          readerStore.setCurrentLocation(paneIndex, {
            scrollRatio: positionRatio,
            switchingMode: true  // Flag to indicate mode switch
          })
